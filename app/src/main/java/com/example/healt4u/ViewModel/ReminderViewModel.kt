...
class ReminderViewModel(
    ...
    fun markTaken(context: Context, log: ReminderLog, patientId : Int = 0) {
        viewModelScope.launch(Dispatchers.IO) {
                    ...
                    if (medicine != null) {
                        ...
                        if (currentQuantityLeft > 0) {
                            ...
                            val updatedMedicine = medicine.copy(
                                quantityLeft = currentQuantityLeft - 1
                            )
                            ...
                            Log.d(
                                ...
                                "Medicine ${medicine.name_medicine}: " +
                                        "$currentQuantityLeft -> ${updatedMedicine.quantityLeft}"
                            )
                            ...
                            Log.d(
                                "ReminderViewModel",
                                "Medicine ${medicine.name_medicine} has no stock left"
                            )
                        }
                    }
                }
    ...
    fun addAppointmentReminder(
        ...
        date: String,
        time: String,
        patientId: Int = com.example.healt4u.Session.CurrentSession.patientId
    ) {
        viewModelScope.launch(Dispatchers.IO) {
            val reminderLog = ReminderLog(
                id = "appt_${patientId}_${date}_${time}_${doctorName.hashCode()}",
                medicineId = -1,
